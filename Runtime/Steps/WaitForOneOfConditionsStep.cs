...
{
    [Serializable]
    public class WaitForOneOfConditionsStep : GameObjectTaskStep
    {
        ...
        public override async Task PerformStep(CancellationToken cancellationToken)
        {
            ...
            try
            {
                await WaitForOneOfConditions(cancellationToken, conditions.ToArray());
            }
            catch (TaskCanceledException)
            {
                Debug.Log("Waiting for one of booleans was cancelled.");
            }
        }
